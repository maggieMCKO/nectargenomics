...
# allcnees = read_tsv("allcnees.tsv") # based on linear/cnee_ncbigene100kb_intersect_allcnees_2.bed
...
# nectar
target_file = list.files(paste0("/Users/mbp23/ownCloud/Baldwin/Projects/Nectar/Seq_Data/globus/CNEEanalysis_2021/03_phyloacc/2024ver/nectar_nodollo/great/overlap/Loose/"), 
...
acccnees_100kb_withcnees_loose = acccnees_100kb_withcnees0_loose %>% 
  ...
  distinct()
dim(acccnees_100kb_withcnees_loose) # 7559    5
...
n_distinct(acccnees_100kb_withcnees0_loose_sel$id) # 11599
...
```{r}
# absrel list
path = paste0("/Users/mbp23/ownCloud/Baldwin/Projects/Nectar/Seq_Data/globus/CNEEanalysis_2021/03_phyloacc/2024ver/nectar_nodollo/combined/othersources/Suppl_Tables_v10.absrel.xlsx") # 2024 updated
path = paste0("/Users/mbp23/ownCloud/Baldwin/Projects/Nectar/Seq_Data/globus/CNEEanalysis_2021/03_phyloacc/2024ver/nectar_nodollo/combined/othersources/Suppl_Tables_v11.absrel.xlsx") # 2024 updated
...
absrel = absrel0 %>% 
  ...
  distinct() %>% 
  filter(!is.na(gene))
n_distinct(absrel$gene) # 1030 # nect only
...
# load data - t2d - 1
```{r}
t2d_anno = read_tsv("t2d_evidence_merged_afterdebug_forPerm.tsv") %>% 
  mutate(gene = gsub("AC012354.6", "SIX3", gene),
         gene = gsub("AC122129.1", "RAI1", gene),
         gene = gsub("CTA-85E5.10", "HORMAD2", gene),
         gene = gsub("INS-IGF2", "INS", gene), # no need becuase we dont have INS anyway
         gene = gsub("LOC107051846", "IRS1", gene))

# t2d_anno = read_tsv(paste0("/Users/mbp23/ownCloud/Baldwin/Projects/Nectar/Seq_Data/globus/CNEEanalysis_2021/03_phyloacc/2024ver/nectar_nodollo/T2D/", "t2d_evidence_merged_afterdebug_forPerm.tsv"))

table(t2d_anno$evidence)
unique(t2d_anno$evidence)
t2d_annos = t2d_anno %>% filter(evidence == "strong") %>% 
  dplyr::select(-id) %>% 
  left_join(allcnees, by = 'gene') # associated cnees
...
t2d_annow = t2d_anno %>% filter(evidence == "weak") %>% 
  dplyr::select(-id) %>% 
  left_join(allcnees, by = 'gene') # associated cnees
n_distinct(t2d_annow$gene)

t2d_anno_union = t2d_anno %>% distinct(gene, .keep_all = T) %>% 
  dplyr::select(-id) %>% 
  left_join(allcnees, by = 'gene') # associated cnees

# t2d_anno_union = t2d_anno %>% distinct(gene, .keep_all = T) %>% 
#   left_join(acccnees_100kb_withcnees0_loose_sel, by = "gene") %>% 
#   left_join(absrel_nect, by = c("gene", "group"))# associated cnees
# 
# t2d_anno_union = t2d_anno_union %>% 
#   filter(!is.na(group)) %>% 
#   group_by(gene) %>% 
#   mutate(n_conv = n_distinct(group))
# # 0.3225806 30/93
# x = final_briefCW %>% filter(gene %in% t2d_annow$gene)
...
t4 = read_excel(path, sheet = 4, skip = 2) %>% clean_names() %>% 
  filter(!is.na(locus)) %>% 
  filter(!is.na(chromosome)) %>% 
  mutate(ori = locus) %>% 
  separate_longer_delim(locus, delim = ", ")
n_distinct(t4$locus) # 849

# combine the first dataset
t4c = t4 %>% full_join(t2d_anno, by = c("locus" = "gene")) %>% 
  mutate(fixed_effects_or_95_percent_ci = gsub(" \\(.*", "", fixed_effects_or_95_percent_ci))
t4c$evidence = factor(t4c$evidence, levels = c("strong", "intermediate", "weak", NA))
...
t4c_75cnee = t4c %>% 
  filter(risk_allele_frequencya > 0.75) %>% 
  dplyr::select(-id) %>% 
  dplyr::rename(gene = locus) %>% 
  left_join(allcnees, by = 'gene') # associated cnees
...
t4_only_cnee = t4 %>% 
  dplyr::rename(gene = locus) %>% 
  left_join(allcnees, by = 'gene') # associated cnees
n_distinct(t4_only_cnee$gene) # 849

t4_only_75cnee = t4 %>% 
  filter(risk_allele_frequencya > 0.75) %>% 
  dplyr::rename(gene = locus) %>% 
  left_join(allcnees, by = 'gene') # associated cnees
n_distinct(t4_only_75cnee$gene) # 175
...
n_distinct(x$ori) # 74 loci
n_distinct(x$gene) # 79 genes
...
l = list("74 loci (79 genes)" = unique(x$gene), "absrel+" = absrel_nect$gene)
ggvenn(l, show_percentage = F)  # 1 signal
...
pdf(graph_path, width = Width_HalfCol*0.65, height = Width_HalfCol*0.375, pointsize = AxisTxFontSizeSize, onefile = TRUE) 
p
dev.off()
openfile(graph_path)
...
# @ square plot - absrel, phyloacc, with control
```{r}

nectar_acc1 = nectar_acc %>% 
  dplyr::select(-id) %>% distinct() %>% 
  mutate(val = 1, analysis = "PhyloAcc", class = "nectar")
nonnectar_acc1 = nonnectar_acc %>% 
  dplyr::select(-id) %>% distinct() %>% 
  mutate(val = 1, analysis = "PhyloAccControl", class = "nonnectar")

absrel_nect1 = absrel_nect %>% mutate(analysis = "aBSREL", class = "nectar")
absrel_nonnect1 = absrel_nonnect %>% mutate(analysis = "aBSRELControl", class = "nonnectar")

combined_nectar = nectar_acc1 %>% bind_rows(absrel_nect1) %>% 
  group_by(gene) %>% 
  mutate(n_clades = n_distinct(group),
         n_ana = n_distinct(analysis)) 
combined_nonnectar = nonnectar_acc1 %>% bind_rows(absrel_nonnect1) 

combined = combined_nectar %>% 
  bind_rows(combined_nonnectar) %>% 
  dplyr::rename(clade = group) %>% 
  mutate(fillcol = analysis) 
dim(combined) # 20433     8

unique(combined$clade)
combined$clade = factor(combined$clade, levels = c("hummingbirds", "parrots", "honeyeaters", "sunbirds",
                                                   "swifts", "falcons", "lyrebirds", "passerides" ))
combined$class = factor(combined$class, levels = c("nectar",
                                                   "nonnectar" ))

# make df for visualizing cubic 
unique(combined$analysis)
combined1 = combined %>% filter(analysis %in% c("aBSREL", "aBSRELControl")) %>% 
  mutate(newlabY = "1")
combined2 = combined %>% filter(analysis %in% c("PhyloAcc", "PhyloAccControl")) %>% 
  mutate(newlabY = "2")

combinedc = combined1 %>% bind_rows(combined2) %>% 
  mutate(newlabX = ifelse(class %in% c("nectar"), "1", "2")) #%>% 
  # distinct()

combinedc = combinedc %>% 
  mutate(newlabZ = case_when(clade %in% c("hummingbirds", "swifts") ~ 1,
                             clade %in% c("parrots", "falcons") ~ 2,
                             clade %in% c("honeyeaters", "lyrebirds") ~ 3,
                             clade %in% c("sunbirds", "passerides") ~ 4,
                            .default = NA)) %>% 
  # make complete so missing values will have a grey edge
  ungroup() %>% 
  tidyr::complete(gene, newlabX, newlabY, newlabZ)

dim(combinedc) # 110160     11
unique(combinedc$newlabX)
unique(combinedc$newlabY)
unique(combinedc$fillcol)

# # sort factors
combinedc$analysis = factor(combinedc$analysis, levels = c("aBSREL", "aBSRELControl", "PhyloAcc", "PhyloAccControl"))
combinedc$fillcol = factor(combinedc$fillcol, levels = c("aBSREL", "aBSRELControl", "PhyloAcc", "PhyloAccControl", NA))
...
col_absrel_control = alpha("#FF7F00", 0.3)  # orange
col_phyloacc_control = alpha("#984EA3", 0.3)  # purple

```

# plot func
```{r}
plot_function_vertical_smallspacing_withcontrol = function(s){
  # s = interested_gene
  # s = c("TIMM22", "AGK")
  print(s)
  input_data_sel = combinedc %>% filter(gene %in% s)
  if(nrow(input_data_sel) == 0){'no gene'}
  input_data_sel$gene = factor(input_data_sel$gene, levels = s)
  
  p = ggplot(input_data_sel, aes(y = newlabX, x = newlabY, fill = fillcol)) +
    # geom_tile(color = 'grey60', linewidth = 0.1) +
    geom_tile(color = 'white', linewidth = 0.1) +
    geom_tile() + # maude likes no internal grid
    facet_grid(gene~newlabZ) +
    scale_y_discrete(limits = rev, expand = c(0,0)) +
    scale_x_discrete(expand = c(0,0)) +
    scale_fill_manual(values = c(col_absrel, col_absrel_control,
                                 col_phyloacc, col_phyloacc_control), 
...
plot_function_vertical_smallspacing_withcontrol(interested_gene2)
```


# Beta cell -PI
```{r}
input = t6 %>% filter(cluster_assignment == "Beta cell -PI") %>% 
  left_join(combined_nectar) %>% 
  arrange(desc(n_clades), desc(n_ana)) #%>% filter(n_clades == 4)
input = input %>% pull(gene) %>% unique()
n_distinct(input) # 131; n_clades==4: 11

p = plot_function_vertical_smallspacing_withcontrol(input)
graph_path = paste0(project_path, "SquarePlor_T2dSuzuki2024_betacell-PI_vert_", Sys.Date(),".pdf"); graph_path
...
p = plot_function_vertical_smallspacing_withcontrol(input)
graph_path = paste0(project_path, "SquarePlor_T2dSuzuki2024_betacellPI_vert_", Sys.Date(),".pdf"); graph_path
...
p = plot_function_vertical_smallspacing_withcontrol(input)
graph_path = paste0(project_path, "SquarePlor_T2dSuzuki2024_bodyfat_vert_", Sys.Date(),".pdf"); graph_path
pdf(graph_path, width = Width_HalfCol*0.25, height = Width_HalfCol*0.006*length(input), pointsize = AxisTxFontSizeSize) # middle, after adding gene list
p
dev.off()
openfile(file = graph_path)
...
p = plot_function_vertical_smallspacing_withcontrol(input)
graph_path = paste0(project_path, "SquarePlor_T2dSuzuki2024_Obesity_vert_", Sys.Date(),".pdf"); graph_path
pdf(graph_path, width = Width_HalfCol*0.25, height = Width_HalfCol*0.006*length(input), pointsize = AxisTxFontSizeSize) # middle, after adding gene list
p
dev.off()
openfile(file = graph_path)
