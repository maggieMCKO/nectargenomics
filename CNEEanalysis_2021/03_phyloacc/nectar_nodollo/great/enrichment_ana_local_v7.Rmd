...
acccnees_100kb_withcnees_looseCon = acccnees_100kb_withcnees0_looseCon %>% 
  ...
  dplyr::select(set, gene, val, analysis, group) %>% 
  distinct()
dim(acccnees_100kb_withcnees_looseCon) # 15437    5
...
all = list.files(paste0(getwd(), "/great_BinomialP_go/"), ".tsv", full.names = T)
...
x = out_go2 %>% group_by(greatset, acc_criteria, accset, ont) %>% 
  tally(); x # should all 17898 (go pathways); BP: 11763/filtered: 662

# FDR
go_sig_padj = out_go2 %>% 
  filter(padj < 0.05) %>% left_join(go_annoC)
print(n_distinct(go_sig_padj$go_id)) # 2929/filtered:388
...
n_distinct(go_sig_padj_path$goTerm) # 2929/filtered:388
...
# focus on biological process
all = list.files(paste0(control_path, "/great_BinomialP_go/"), ".tsv", full.names = T)
all2 = all[grepl("biological_process", all)]
...
x = out_go_con2 %>% group_by(greatset, acc_criteria, accset, ont) %>% 
  tally(); x # should all 17898 (go pathways): 11763 BP/662

go_sig_padj_con = out_go_con2 %>% filter(padj < 0.05) %>% left_join(go_annoC)
print(n_distinct(go_sig_padj_con$go_id)) # 1397/273
...
go_sig_padj_con_path_summary = go_sig_padj_con_path %>% group_by(greatset, acc_criteria, accset, ont) %>% 
  summarise(n = n_distinct(go_id))
n_distinct(go_sig_padj_con_path$goTerm) # 1397/273
...
loose_hypo_onlyoverlap_gteq2 = loose_hypo_marked %>% 
  ...
  filter(n_clades >= 2) %>% 
  filter(sig == "*")
n_distinct(loose_hypo_onlyoverlap_gteq2$go_id) # 53
...
conloose_hypo_onlyoverlap_gteq2 = conloose_hypo_marked %>% 
  ...
  filter(n_clades >= 2) %>% 
  filter(sig == "*")
n_distinct(conloose_hypo_onlyoverlap_gteq2$go_id) # 124/37/34
```
...
nectar_only2 = nectar_only %>% 
  ...
  filter(n_clades >= 3) %>% 
  mutate(go_id2 = gsub("_", ":", go_id))
n_distinct(nectar_only2$go_id) # 4 clades: 0; 3 clades: 5
...
graph_path = paste0(out_dir, "GO_", fdr_flag,"_", tmp_acc_criteria, "_", tmp_accset, "_rep_nocontrol_withgene_greathypogeometric_p1_", Sys.Date(), ".pdf")
pdf(graph_path, width = Width_HalfCol*3.1, height = Width_HalfCol*0.275, pointsize = AxisTxFontSizeSize, onefile = TRUE)
pl
dev.off()
...
control_only2 = control_only %>% 
  ...
  filter(n_clades >= 3) %>%
  mutate(go_id2 = gsub("_", ":", go_id))
n_distinct(control_only2$go_id) # 4 clades: 0; 3 clades: 1
