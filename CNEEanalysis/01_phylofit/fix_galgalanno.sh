...
onlyCDS=${input_ann%%.gff}_chrtabFixed_CDS.gff
...
sed 's/\r$//g' GCF_000002315.6_GRCg6a_assembly_report.txt | grep -v "^#" | cut -f7,10 | awk '{gsub(/chr/, "galGal6.chr") ; print}' | awk '{gsub(/\tna/, "\tgalGal6.MT") ; print}' > ${trans_matrix}
